...
def check_statistical_outliers(df, label):
    """Detect statistical outliers using IQR method"""
    print(f"\n--- {label} Statistical Outlier Detection ---")
    
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    for col in numeric_cols:
        if col in ['datetime_utc']:  # Skip timestamp columns
            continue
        ...
        if not outliers.empty:
            ...
            print(f"    Expected range: [{lower_bound:.3f}, {upper_bound:.3f}]")
        else:
...
def print_datetime_regularity(df, label):
    ...
    check_statistical_outliers(df, label)
...
def main():
    ...
    # Add cross-table consistency check
    check_cross_table_consistency(config)
